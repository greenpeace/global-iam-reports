...
class AuditReport implements ReportDataSetBuilder {
    ...
    ReportDataDto getReportData(ReportQueryDto query) {
        ...
        def String action_result = query.getParameterValue("ACTION_RESULT")
        def String action_result_full = query.getParameterValue("ACTION_RESULT_FULL")
        ...
        if (outputTable == "HEAD") {
            ...
            if (action_result) {
                if (action_result == "Y") {
                    row.column.add(new ReportColumn('ACTION_RESULT_FULL', "SUCCESS"))
                }
                if (action_result == "N") {
                    row.column.add(new ReportColumn('ACTION_RESULT_FULL', "FAILURE"))
                }
            }
        ...
        if (logId) {
            ...
            if (action_result == "Y") {
                logSearchBean.result = "SUCCESS"
            }
            if (action_result == "N") {
                logSearchBean.result = "FAILURE"
            }
            logSearchBean.sortBy = [new SortParam(OrderConstants.DESC, "timestamp")] as List
            auditLogs = auditLogService.findBeans(logSearchBean, 0, 1000, false)
        }
    ...
    private void addMainTableRow(IdmAuditLog log, ReportTable reportTable, String parentActionId) {
        ...
        def user = null
        if(!a.userId) {
            row.column.add(new ReportColumn('REQ_FULLNAME', "system"))
        } else {
            user = getUser(a.userId)
            if(user == null) {
                Login l = loginDataWebService.getPrimaryIdentity(a.userId)?.getPrincipal()
                row.column.add(new ReportColumn('REQ_FULLNAME', l?.login))
            } else {
                for (UserEntity u : user) {
                    def String userName11 = u.firstName + " " + u.lastName
                    userName11 = userName11.trim().length()>0 ? userName11 : loginDataWebService.getPrimaryIdentity(a.userId).getPrincipal().getLogin()
        ...
        if (a.result == "FAILURE") {
            def attr = a.getCustomRecords().find({ it.key = "FAILURE_REASON" })
            row.column.add(new ReportColumn('ERROR_REASON', attr.value))
        }
